...
const { postedEsanaNews, get_EsanaPosted, postedHiruNews, get_HiruPosted } = require("./database/posted_news")
const { fetchauth, updateAuth } = require("./database/session")
const news_groups = [
    "120363067177677604@g.us",
    "120363029046347577@g.us"
]
...
const ESANA_BOT = async () => {
    ...
    setInterval(async () => {
        ...
        console.log('Checking new Hiru news available...')

        // ===== Hiru News =====
        const hiru = require("./lib/hirunews")
        const latest_hiru_data = await hiru.latestHiru();
        const PostedHN = await get_HiruPosted();

        if (PostedHN.hiruNews === latest_hiru_data.link) {
            console.log('No new Hiru news yet!')
        } else {
            try {
                console.log(`Posting ${latest_hiru_data.link}`);
                await postedHiruNews(latest_hiru_data);
                const dbNewsData = await get_HiruPosted();
                const latest_news = JSON.parse(dbNewsData.newsData);
                for (const jid of news_groups) {
                    const image = await sock.sendMessage(jid, { image: { url: latest_news.img }, caption: `*${latest_news.title}*\n\n${latest_news.desc}\n\n[${latest_news.link}]` });
                    const reactionMessage = { react: { text: "🗞️", key: image.key } };
                    await sock.sendMessage(jid, reactionMessage);
                }
                return;
...
if (process.env.DYNO) {
    setInterval(async () => {
        await got(`https://${process.env.APP_NAME}.herokuapp.com/`);
        l("pinging...")
    }, 1000 * 60 * 20)
}
